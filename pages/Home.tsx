...
import { ArrowRight, Heart, CheckCircle, X } from 'lucide-react';
...
export const Home: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-transparent">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold animate-pulse" style={{ color: BRAND_BLUE }}>Loading content...</p>
        </div>
      </div>
    );
  }
  ...
  const itemVariants = { hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { type: "spring", stiffness: 50, damping: 20 } } };
  const headlineContainerVariants = { hidden: { opacity: 1 }, visible: { opacity: 1, transition: { delayChildren: 0.2, staggerChildren: 0.1 } } };
  const wordVariants = { hidden: { opacity: 0, y: 40, filter: "blur(10px)", scale: 1.1 }, visible: { opacity: 1, y: 0, filter: "blur(0px)", scale: 1, transition: { duration: 0.8, ease: [0.2, 0.65, 0.3, 0.9] } } };
  ...
  return (
    <div className="w-full overflow-hidden bg-transparent">
      {/* HERO SECTION */}
      <section className="relative h-[100vh] min-h-[700px] flex items-center justify-center overflow-hidden" style={{ backgroundColor: BRAND_BLUE }}>
        <div className="absolute inset-0 z-0 overflow-hidden">
          <motion.div style={{ y: y1 }} className="absolute inset-0 w-full h-[120%]">
            <motion.img
              {...({ initial: { scale: 1.2, opacity: 0 }, animate: { scale: 1, opacity: 0.3 }, transition: { duration: 1.5, ease: "easeOut" } } as any)}
              src={content.hero.heroImage}
              alt="OBOMOCARE Community"
              className="w-full h-full object-cover"
            />
          </motion.div>
          <div className="absolute inset-0" style={{ background: BRAND_BLUE, opacity: 0.7 }}></div>
          <div className="absolute top-[-60px] right-[-60px] w-[300px] h-[300px] rounded-full" style={{ background: `rgba(232,117,26,0.12)` }}></div>
          <div className="absolute bottom-[-40px] left-[20%] w-[180px] h-[180px] rounded-full" style={{ background: `rgba(232,117,26,0.07)` }}></div>
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 text-center sm:px-6 lg:px-8 mt-12">
          <motion.div {...({ initial: "hidden", animate: "visible", variants: containerVariants } as any)}>
            ...
            <motion.h1
              {...({ variants: headlineContainerVariants, initial: "hidden", animate: "visible" } as any)}
            ...
            >
              {headlineWords.map((word, index) => (
                <motion.span key={index} variants={wordVariants} className="inline-block mr-[0.25em] last:mr-0">
                  {word}
                </motion.span>
              ))}
            </motion.h1>
            ...
            <motion.div {...({ variants: itemVariants } as any)} className="flex flex-col sm:flex-row gap-5 justify-center">
              <motion.div {...({ whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, transition: { type: "spring", stiffness: 400, damping: 15 } } as any)}>
                <ReactRouterDOM.Link
                  ...
                  style={{ backgroundColor: BRAND_ORANGE, borderColor: BRAND_ORANGE }}
                >
      ...
      {/* STATS BAR */}
      <section className="relative z-20 px-4 mb-32" style={{ backgroundColor: BRAND_ORANGE }}>
        <motion.div
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center gap-20">
            ...
            >
              ...
              <ReactRouterDOM.Link to="/about" className="group flex items-center gap-3 text-lg font-bold transition-all" style={{ color: BRAND_BLUE }}>
            ...
            <div className="lg:w-1/2 grid grid-cols-2 gap-6 relative">
              ...
              <div className="absolute -bottom-10 -left-10 w-32 h-32 rounded-full blur-[60px] opacity-40 pointer-events-none" style={{ backgroundColor: BRAND_ORANGE }}></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 relative" style={{ backgroundColor: SURFACE_1 }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {content.programs.map((program) => (
              <motion.div key={program.id} {...({ variants: itemVariants } as any)}>
                  ...
                  <GlassCard className="h-full flex flex-col group" hoverEffect={true}>
                    <div className="h-56 overflow-hidden relative">
                      ...
                      <div className="absolute bottom-6 left-6 right-6">
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-2" style={{ backgroundColor: BRAND_ORANGE }}>
                          {program.stats}
                        </span>
                        <h3 className="font-serif text-2xl font-bold text-white leading-tight">{program.title}</h3>
                      </div>
                    </div>
                    <div className="p-6 bg-white/60 flex flex-col flex-grow">
                      <p className="text-slate-600 text-sm mb-4 flex-grow leading-relaxed line-clamp-3">
                        {program.description}
                      </p>
                      <span className="inline-flex items-center gap-2 font-bold text-sm group-hover:gap-3 transition-all" style={{ color: BRAND_BLUE }}>
                        Learn more <ArrowRight size={14} style={{ color: BRAND_ORANGE }} />
                      </span>
                    </div>
                  </GlassCard>
                </ReactRouterDOM.Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <motion.div
                ...
                style={{ borderTop: `3px solid ${BRAND_ORANGE}`, backgroundColor: '#fff' }}
                {...({
              ...
              >
                <h4 className="text-sm font-medium mb-3" style={{ color: BRAND_BLUE }}>{card.title}</h4>
      ...
      {/* THEORY OF CHANGE */}
      <section className="py-24" style={{ backgroundColor: BRAND_BLUE_LIGHT }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16">
            <div className="inline-block rounded-full px-4 py-2 text-xs font-bold tracking-[0.15em] uppercase mb-6" style={{ backgroundColor: BRAND_ORANGE_LIGHT, color: BRAND_ORANGE }}>
              Theory of change
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center mb-8">
            ...
            <div className="hidden lg:flex justify-center">
              <div className="text-4xl" style={{ color: BRAND_ORANGE }}>→</div>
            </div>
            <div className="rounded-xl p-8 text-center text-white" style={{ backgroundColor: BRAND_BLUE }}>
            ...
            {['Improved clinic attendance', 'Better nutrition', 'Restored dignity', 'Reduced isolation', 'Household wellbeing', 'Community resilience'].map((item, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  <span className="px-4 py-2 rounded-md text-sm font-medium" style={{ backgroundColor: isLast ? BRAND_BLUE : BRAND_BLUE_LIGHT, color: isLast ? '#fff' : BRAND_BLUE }}>
                    {item}
                  </span>
                  {i < 5 && <span className="text-lg" style={{ color: BRAND_ORANGE }}>→</span>}
                </React.Fragment>
              );
            })}
          </div>
      ...
      {/* IMPACT TARGETS / KPIs */}
      <section className="py-24" style={{ backgroundColor: SURFACE_1 }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16">
            <div className="inline-block rounded-full px-4 py-2 text-xs font-bold tracking-[0.15em] uppercase mb-6" style={{ backgroundColor: BRAND_ORANGE_LIGHT, color: BRAND_ORANGE }}>
              Impact targets
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              >
                <div className="text-3xl font-medium mb-2" style={{ color: BRAND_ORANGE }}>{kpi.num}</div>
    ...
    <section className="py-24" style={{ backgroundColor: '#fff' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            <div className="rounded-xl p-6 text-white mb-6" style={{ backgroundColor: BRAND_BLUE }}>
            ...
            <div className="rounded-xl p-6 border border-slate-200" style={{ backgroundColor: SURFACE_1 }}>
              <p className="text-xs text-slate-500 mb-3 font-medium">Current volunteer base</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-medium" style={{ color: BRAND_ORANGE }}>22</div>
                  <div className="text-xs text-slate-400">Active caregivers</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-medium" style={{ color: BRAND_ORANGE }}>18</div>
    ...
    <section className="py-24" style={{ backgroundColor: SURFACE_1 }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="rounded-xl p-8 md:p-12 text-white text-center mb-10" style={{ backgroundColor: BRAND_BLUE }}>
            ...
            <div key={i} className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3 md:gap-6 items-center bg-white p-4 rounded-lg border border-slate-200">
              <div>
                ...
                <div className="h-1.5 bg-slate-100 rounded-full mt-2 overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: line.width, backgroundColor: BRAND_ORANGE }}></div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            const statusColors: Record<string, { bg: string; color: string }> = {
              ...
              growing: { bg: BRAND_BLUE_LIGHT, color: BRAND_BLUE },
              pilot: { bg: BRAND_ORANGE_LIGHT, color: BRAND_ORANGE }
            };
    ...
    {/* CTA */}
    <section className="py-20 text-center" style={{ backgroundColor: BRAND_ORANGE }}>
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <ReactRouterDOM.Link
              ...
              style={{ backgroundColor: '#fff', color: BRAND_ORANGE }}
            >
              Partner with us
            </ReactRouterDOM.Link>
            <ReactRouterDOM.Link
