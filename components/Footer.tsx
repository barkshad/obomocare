...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="text-white font-sans" style={{ backgroundColor: brandBlue }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          {/* Brand */}
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-serif font-bold text-lg" style={{ backgroundColor: brandOrange }}>
                O
              </div>
            ...
            <ul className="space-y-3 text-sm">
              <li>
                <ReactRouterDOM.Link to="/about" className="hover:text-white transition-colors flex items-center gap-2 group">
                  <span className="w-0 group-hover:w-2 h-[1px] transition-all" style={{ backgroundColor: brandOrange }}></span> Who we are
                </ReactRouterDOM.Link>
              </li>
              <li>
                <ReactRouterDOM.Link to="/programs" className="hover:text-white transition-colors flex items-center gap-2 group">
                  <span className="w-0 group-hover:w-2 h-[1px] transition-all" style={{ backgroundColor: brandOrange }}></span> Our work
                </ReactRouterDOM.Link>
              </li>
              <li>
                <ReactRouterDOM.Link to="/stories" className="hover:text-white transition-colors flex items-center gap-2 group">
                  <span className="w-0 group-hover:w-2 h-[1px] transition-all" style={{ backgroundColor: brandOrange }}></span> Impact
                </ReactRouterDOM.Link>
              </li>
  <li>
  <ReactRouterDOM.Link to="/sponsorship" className="hover:text-white transition-colors flex items-center gap-2 group">
  <span className="w-0 group-hover:w-2 h-[1px] transition-all" style={{ backgroundColor: brandOrange }}></span>
  Programme pillars
  </ReactRouterDOM.Link>
  </li>
  <li>
  ...
  Governance & Risk
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm">
              <li className="flex items-start gap-3">
                <MapPin size={18} className="mt-0.5 flex-shrink-0" style={{ color: brandOrange }} />
                <span className="text-white/70">{content.contact.address}</span>
              </li>
              <li className="flex items-center gap-3">
                <Mail size={18} className="flex-shrink-0" style={{ color: brandOrange }} />
                <a href={`mailto:${content.contact.email}`} className="hover:text-white transition-colors" style={{ color: brandOrange }}>
                  {content.contact.email}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Phone size={18} className="flex-shrink-0" style={{ color: brandOrange }} />
                <span className="text-white/70">{content.contact.phone}</span>
              </li>
              <li className="flex items-center gap-3">
                <MessageCircle size={18} className="flex-shrink-0" style={{ color: brandOrange }} />
                <a
        ...
        <div className="border-t mt-16 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-white/50">
