  ...
  return (
    <div className="section" style={{ background: 'var(--surface-primary)', paddingTop: '6rem' }}>
          ...
          {content.children.map((child) => (
            ...
            >
              ...
              <div style={{ padding: 'var(--space-6)' }}>
                ...
                <h3 style={{ fontSize: 'var(--text-xl)', fontWeight: 600, color: '#fff', marginBottom: 'var(--space-2)' }}>{child.name}</h3>
                <p style={{ fontSize: 'var(--text-sm)', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, marginBottom: 'var(--space-4)' }}>{child.bio}</p>
      ...
      {selectedChild && (
        ...
        >
          <div
            style={{ background: 'var(--surface-elevated)', borderRadius: 'var(--radius-md)', maxWidth: '36rem', width: '100%', overflow: 'hidden', position: 'relative' }}
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedChild(null)}
              style={{ position: 'absolute', top: 'var(--space-4)', right: 'var(--space-4)', background: 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', color: '#fff', padding: 'var(--space-2)', borderRadius: '50%', display: 'flex', zIndex: 10 }}
            >
              <X size={20} />
            </button>
            {selectedChild.image && (
            ...
            <div style={{ padding: 'var(--space-8)' }}>
              ...
              <h2 style={{ fontSize: 'var(--text-2xl)', fontWeight: 600, color: '#fff', marginBottom: 'var(--space-4)' }}>{selectedChild.name}</h2>
              <p style={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, marginBottom: 'var(--space-6)' }}>{selectedChild.bio}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', marginBottom: 'var(--space-6)' }}>
                {['Direct household support', 'Community volunteer caregivers', 'Transparent quarterly reporting'].map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center', fontSize: 'var(--text-sm)', color: 'rgba(255,255,255,0.6)' }}>
