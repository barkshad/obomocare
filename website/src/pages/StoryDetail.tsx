...
export const StoryDetail: React.FC = () => {
  ...
  const { content } = ReactRouterDOM.useOutletContext ? ReactRouterDOM.useOutletContext() : useContent();
  const { content: pageContent } = useContent();

  const story = pageContent.stories.find(s => s.id === id);
  ...
  return (
    <div className="section" style={{ background: 'var(--surface-primary)', paddingTop: '6rem' }}>
      <div className="container" style={{ maxWidth: '48rem' }}>
        ...
        <h1 style={{ fontSize: 'var(--text-5xl)', fontWeight: 700, color: '#fff', marginBottom: 'var(--space-4)', lineHeight: 1.1 }}>
          {story.title}
        </h1>

        <div style={{ display: 'flex', gap: 'var(--space-6)', marginBottom: 'var(--space-10)', color: 'rgba(255,255,255,0.5)', fontSize: 'var(--text-sm)' }}>
        ...
        <div style={{ padding: 'var(--space-10)', background: 'var(--surface-elevated)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--space-10)' }}>
          <p style={{ fontSize: 'var(--text-base)', color: 'rgba(255,255,255,0.85)', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
            {story.content}
          </p>
        </div>
